...
class selectionSL():
    ...
    def run(self, df):
        ...
        df = df.Define('MuonLepSign','SSOS(MuonGoodInd, ElectronGoodInd, MuonJetInd, Electron_charge, Muon_charge)')
        df = df.Define('weightSSOS','-1*MuonLepSign')
        ...
        #df = df.Filter('nMuonGood<2 && nElectronGood<2').Filter('!(nMuonGood==1) != !(nElectronGood==1)').Filter('nJetGood>=4').Filter('MuonJetInd.size() >= 1').Filter('MuonJetGood')
        #df = df.Define('JetnotMuonInd','secondjet(nJetGood, JetGoodInd, JetMuonInd, JetBotInd, Jet_pt_nom)')
        ### hists definitions
        ...
        variables = ['MuonGoodInd','ElectronGoodInd','nMuonGood','nElectronGood','JetGoodInd','nJetGood','JetBotInd','MuonJetInd','JetMuonInd','MuonJetGood','MuonLepSign','weightSSOS',
           'lepton_type','lepton_pt','lepton_eta','transverse_mass','lepton_iso'
        ]
...
def selectionSLRDF(**kwargs):
    ...
    """
    return lambda: selectionSL(**kwargs)
