...
gInterpreter.Declare("""
      ...
      using Vfloat = const ROOT::RVec<float>&;
      using Vint = const ROOT::RVec<int>&;
      auto SSOS_mu(Vint mu_good, Vint el_good, Vint mu_jet,Vint el_charge, Vint mu_charge) {
            int ind = 0;
            if(mu_jet.size()>0){
                if(mu_good.size()>0){
...
class selection_general():
    def __init__(self, *args, **kwargs):
        ...
        if (not self.ispreVFP) and self.year==2016:
           self.tag_year = "2016B"
        else:
       	   self.tag_year = str(self.year)
    ...
    def run(self, df):
        ...
        df = df.Define('MuonGoodInd','muonInd(nMuon,Muon_pt,Muon_eta,Muon_pfRelIso04_all, Muon_tightId,'+str(muon_pt[self.tag_year])+')')
        df = df.Define('ElectronGoodInd','elInd(nElectron, Electron_pt, Electron_eta, Electron_pfRelIso03_all, Electron_cutBased, Electron_mvaFall17V2Iso_WP80, Electron_mvaFall17V2Iso_WP90,'+str(el_pt[self.tag_year])+')')
        ...
        df = df.Define('MuonLepSign','SSOS_mu(MuonGoodInd, ElectronGoodInd, MuonJetInd, Electron_charge, Muon_charge)')
        ...
        df = df.Filter(met_filter[self.tag_year])
        df = df.Define('lepton_type','nMuonGood>0 ? 0 : 1')
        if self.isMC:
                df = df.Filter('nMuonGood>0 ? '+ str(muon_trig[self.tag_year]) +' : '+ str(el_trig[self.tag_year]))
        else:
                if self.isMU:
                        df = df.Filter(muon_trig[self.tag_year])
                else:
                        if str(self.tag_year) == "2017":
                                ...
                                df = df.Filter(el_trig[self.tag_year])
        ...
        #df = df.Filter('jet_bot1_btag >'+str(cuts_btag[tag_year][1]))
        #df = df.Filter('jet_bot2_btag >'+str(cuts_btag[tag_year][0]))

        variables = ['MuonGoodInd','ElectronGoodInd','nMuonGood','nElectronGood','JetGoodInd','nJetGood','JetBotInd','JetSVInd','SVJetInd','SVJetGood','SVLepSign',
           'MuonJetInd','JetMuonInd','MuonJetGood','lepton_type','lepton_pt','lepton_eta','transverse_mass','lepton_iso', 'MuonLepSign'        
        ]
...
def selection_generalRDF(**kwargs):
    """
    ...
    """
    return lambda: selection_general(**kwargs)
